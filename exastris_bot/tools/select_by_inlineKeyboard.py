...
__available_callback_map: dict[str, int] = {}
...
async def select_value_by_inline_keyboard(bot: Bot,
        ...
        for j in range(len(selection[i])):
            ...
            inline_keyboard[-1].append(
                InlineKeyboardButton(text=selection[i][j][0],
                                     callback_data=f"{UNIVERSAL_CALLBACK_PREFIX},{callback_id},{counter + 1},{control}"))
            counter += 1
    ...
    __available_callback_map[callback_id] = 0
    ...
    if __available_callback_map[callback_id]:
