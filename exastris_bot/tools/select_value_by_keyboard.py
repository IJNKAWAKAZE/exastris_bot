    ...
    for i in range(len(selection)):
        ...
        for j in range(len(selection[i])):
            ...
            inline_keyboard[-1].append(
                InlineKeyboardButton(text=selection[i][j][0], callback_data=f"{callback_id},{counter+1},{control}"))
            counter += 1
...
async def universal_callback_handler(update: Update, bot: Bot):
    callback_id,user_select_str,access_control_str = update.callback_query.data.split(",")
    if callback_id in callback_map:
        user_id = callback_map[callback_id]
        if update.effective_user.id == user_id:
            state = ClickControl.MAKER
        elif (await bot.get_chat_member(chat_id=update.effective_chat.id, user_id=update.effective_user.id)).status==ChatMemberStatus.ADMINISTRATOR:
